-- Complete Setup Script for Nyaya AI
-- This script creates all necessary tables and policies without conflicts

-- ========================================
-- 1. KYR ARTICLES TABLE
-- ========================================

-- Create KYR articles table if it doesn't exist
...
CREATE TABLE IF NOT EXISTS bookmarks (
    ...
    article_id UUID REFERENCES kyr_articles(id) ON DELETE CASCADE,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
DROP POLICY IF EXISTS "Users can create their own documents" ON generated_documents;
DROP POLICY IF EXISTS "Users can update their own documents" ON generated_documents;
...
-- ========================================
-- 6. INSERT SAMPLE KYR ARTICLES (if not exists)
-- ========================================

-- Insert sample KYR articles only if they don't exist
INSERT INTO kyr_articles (title, slug, category, summary, content, read_time, difficulty, tags) 
SELECT 
...
## Overview
Article 14 of the Indian Constitution guarantees the right to equality before law and equal protection. This fundamental right ensures that all citizens are treated equally by the state, regardless of their religion, race, caste, sex, or place of birth.
...
WHERE NOT EXISTS (
    SELECT 1 FROM kyr_articles WHERE slug = 'right-to-equality-article-14'
);
...
WHERE NOT EXISTS (
    SELECT 1 FROM kyr_articles WHERE slug = 'right-to-freedom-of-speech-article-19'
);
...
WHERE NOT EXISTS (
    SELECT 1 FROM kyr_articles WHERE slug = 'right-to-education-article-21a'
);
...
WHERE NOT EXISTS (
    SELECT 1 FROM kyr_articles WHERE slug = 'right-to-information-act-2005'
);
...
WHERE NOT EXISTS (
    SELECT 1 FROM kyr_articles WHERE slug = 'consumer-protection-act-2019'
);
...
SELECT 'KYR Articles' as table_name, COUNT(*) as record_count FROM kyr_articles
UNION ALL
SELECT 'Bookmarks' as table_name, COUNT(*) as record_count FROM bookmarks
UNION ALL
SELECT 'Chat Sessions' as table_name, COUNT(*) as record_count FROM chat_sessions
UNION ALL
SELECT 'Chat Messages' as table_name, COUNT(*) as record_count FROM chat_messages
UNION ALL
SELECT 'Generated Documents' as table_name, COUNT(*) as record_count FROM generated_documents;
